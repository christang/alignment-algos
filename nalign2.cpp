...
void usage();
void smooth_subopt(SuboptFlags&);
...
int main ( int argc, const char** argv ) {
  ...
  try {
    ...
    Hmap2Eval ge (ali_params);                 // New for gn2
    ...
    DPMatrix<HMAPSequence,SMAPSequence,Hmap2Eval> dpm (query, templ, ge, 
						     fwd);

    clock_t t1 = clock();

    Optimal<HMAPSequence,SMAPSequence,Hmap2Eval> opt;
    AlignmentSet<HMAPSequence,SMAPSequence,Hmap2Eval> alignments (dpm, opt);
    ...
    if (!optflag) {
      if (ucwflag) {
        ...
        UnconstrainedNearOptimal<HMAPSequence,SMAPSequence,Hmap2Eval>
          ucw (ali_params);
	...
	KSConstrainedNearOptimal<HMAPSequence,SMAPSequence,Hmap2Eval>
	...
	kscno.enumerate (dpm,alignments);
	  ...
	  //cerr << "Smoothing suboptimal region. (Removing SSEs of length < 2)." << endl;
	  //smooth_subopt (subopt);
	}
	...
	// Needs k_limit, user_limit, max_overlap, rounds; now in 
        CRConstrainedNearOptimal<HMAPSequence,SMAPSequence,Hmap2Eval>
        ...
        ConstrainedNearOptimal<HMAPSequence,SMAPSequence,Hmap2Eval> 
...
void smooth_subopt (SuboptFlags& sf) 
{
