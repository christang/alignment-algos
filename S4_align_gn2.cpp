...
int main ( int argc, const char** argv ) {
  ...
  try {
    ...
    if( args.find( "max_in_betw_shift" ) ) {
      max_in_betw_shift = atof( args.getValue( "max_in_betw_shift" ).str().c_str() );
    }
    ...
    Gn2Eval akev (ali_params);

    LogisticNormal ln (query.evd1_field,query.evd2_field,
		       templ.evd1_field,templ.evd2_field);

    DPMatrix<HMAPSequence,SMAPSequence,Gn2Eval> dpm_fwd( query, templ, akev, 
							   fwd );
    ...
    AlignmentSet<HMAPSequence,SMAPSequence,Gn2Eval> alignments( dpm_fwd, opt );

    alignments.clear();

    cerr << "About to instantiate s_four" << endl;

    SSSS<HMAPSequence,SMAPSequence,Gn2Eval> s_four ( ali_params, akev, &dpm_fwd,
...
void usage() {
  ...
  cerr << endl << "Usage: ";
  cerr << "AK_ali query.prof template.prof --max_returned 500 --min_CO 0.5" << endl;
  cerr << endl;
  ...
  cerr << "   --max_in_betw_shift .. neighborhood width when searching for in-between fragment (2)" << endl;
